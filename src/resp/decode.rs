...
impl RespDecode for RespFrame {
    ...
    fn decode(buf: &mut BytesMut) -> Result<Self, RespError> {
        ...
        match iter.peek() {
            ...
            Some(b'#') => {
                let frame: bool = bool::decode(buf)?;
                Ok(frame.into())
            }
            Some(b',') => {
...
fn parse_length(buf: &[u8], prefix: &str) -> Result<(usize, usize), RespError> {
    ...
    // 已知返回类型 usize， 调用 s.parse()，rust 会试图将 s 解析成 usize
    Ok((end, s.parse()?))
}

fn calc_totoal_length(
    buf: &[u8],
    end: usize,
    len: usize,
    prefix: &str,
...
mod tests {
    ...
    fn test_calc_array_length() -> Result<()> {
        ...
        let total_len = calc_totoal_length(buf, end, len, "*")?;
